...
pub fn voxel_buffer(
    data: Vec<VoxelData>,
    allocator: Arc<GenericMemoryAllocator<Arc<FreeListAllocator>>>,
) -> Arc<CpuAccessibleBuffer<[VoxelData]>> {
...
pub fn material_buffer(
    data: Vec<Material>,
    allocator: Arc<GenericMemoryAllocator<Arc<FreeListAllocator>>>,
) -> Arc<CpuAccessibleBuffer<[Material]>> {
    CpuAccessibleBuffer::from_iter(
        allocator.as_ref(),
        BufferUsage {
            storage_buffer: true,
...
pub fn render_buffer(vulkan_data_rw: Arc<RwLock<Option<VulkanData>>>) -> RenderBufferData {
    ...
    let buffer = CpuAccessibleBuffer::from_iter(
        ...
        false,
        (0..vulkan_data.images[0].dimensions().width()
            * vulkan_data.images[0].dimensions().height()
            * 4)
            .map(|_| 0u8),
    )
    .unwrap();
