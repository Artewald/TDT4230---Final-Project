...
pub fn render_shader(device: Arc<Device>) -> Arc<ShaderModule> {
    mod cs {
        vulkano_shaders::shader! {
            ...
            const uint AMOUNT_OF_PRIMARY_RAYS = 9;
            ...
            bool slabs(VoxelData voxel, Ray ray, vec3 invertedRayDirection, out vec3 intersectionPoint, out vec3 normal) {
                ...
                intersectionPoint = ray.origin + ray.direction * tNear;//tFar;//
            ...
            float get_distance(Ray ray, Hit hit) {
                return length(hit.point - ray.origin);
            }
            ...
            bool is_closer(in Ray ray, in Hit hit, in float closest) {
                return get_distance(ray, hit) < closest;
            }
            ...
            Hit voxel_hit(Ray ray, vec4 clear_col) {
                ...
                if (!slabs(temp_voxel, ray, invRaydir, ret_val.point, ret_val.normal)) return ret_val;
                ...
                for (int i_0 = 0; i_0 < level_0.length(); i_0++) {
                    ...
                    if (!slabs(temp_voxel, ray, invRaydir, ret_val.point, ret_val.normal) || !is_closer(ray, ret_val, closest)) continue;
                    if (is_leaf_node(temp_voxel)) {
                        ret_val = fill_hit_color(temp_voxel);
                        closest = get_distance(ray, ret_val);
                        continue;
                    }
            ...
            void main() {
                ...
                for (int i = 0; i < AMOUNT_OF_PRIMARY_RAYS; i++) {
                    ...
                    if (hit.hit) {
                        color = hit.color;//vec4(normalize(hit.point), 1.0);
                        // break;
                    }
