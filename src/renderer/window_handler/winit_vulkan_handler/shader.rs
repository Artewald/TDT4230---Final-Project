    ...
    mod cs {
        vulkano_shaders::shader! {
            ...
            Ray get_primary_ray(float x_offset, float y_offset) {
                ...
                const vec2 pixel_NCD = vec2((float(IDxy.x)+x_offset)/float(screenSize.x), (float(IDxy.y)+y_offset)/float(screenSize.y));
            ...
            void main() {
                ...
                float offsets[3] = float[](0.25, 0.5, 0.75);
            
                for (int x = 0; x < offsets.length(); x++) {
                    for (int y = 0; y < offsets.length(); y++) {
                        const Ray ray = get_primary_ray(offsets[x], offsets[y]);
                        color_in_the_end += voxel_hit(ray, camera.clear_color).color * (1.0 / (offsets.length()*offsets.length()));
                    }
                }
