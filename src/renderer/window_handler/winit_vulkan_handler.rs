use std::{sync::{RwLock, Arc}, time::Instant, f32::consts::PI};

use artewald_engine_lib::voxel::VoxelData;
use nalgebra::{Vector3, Vector4};
use vulkano::{pipeline::{ComputePipeline, Pipeline, PipelineBindPoint}, sync::{self, GpuFuture, FlushError}, swapchain::{SwapchainCreateInfo, SwapchainCreationError, acquire_next_image, AcquireError, SwapchainPresentInfo}, command_buffer::{AutoCommandBufferBuilder, CopyBufferToImageInfo, CopyImageToBufferInfo, CopyImageInfo, CommandBufferUsage}, image::{ImageAccess, ImageViewAbstract}, descriptor_set::{allocator::StandardDescriptorSetAllocator, layout::{DescriptorSetLayout, DescriptorType}, PersistentDescriptorSet, WriteDescriptorSet}, buffer::CpuAccessibleBuffer};
use winit::{event_loop::{EventLoop, ControlFlow}, event::{Event, WindowEvent, VirtualKeyCode, ElementState}, dpi::{PhysicalPosition, PhysicalSize}};
...
impl WindowHandler for WinitVulkanHandler {
    fn run(&mut self, inital_voxels_to_present: Vec<VoxelData>) {
        ...
        vulkan_data_reference.write().unwrap().replace(VulkanData::new_default(&event_loop));

        let main_shader = render_shader(vulkan_data_reference.read().unwrap().as_ref().unwrap().device.clone());

        let compute_pipline = ComputePipeline::new(
            vulkan_data_reference.read().unwrap().as_ref().unwrap().device.clone(), 
        ...
        let camera_data_buffer = camera_data_buffer(Camera::new(90, 1000.0, (vulkan_data_reference.read().unwrap().as_ref().unwrap().window.inner_size().width as f32)/(vulkan_data_reference.read().unwrap().as_ref().unwrap().window.inner_size().height as f32), Vector3::new(0.0, 0.0, 1.0), Vector3::new(0.0, 1.0, 0.0), Vector4::new(0.0, 0.0, 0.1884, 1.0), Vector3::new(-2.0, 0.0, 0.0)), vulkan_data_reference.read().unwrap().as_ref().unwrap().memory_allocator.clone());
        let voxel_buffer = voxel_buffer(inital_voxels_to_present, vulkan_data_reference.read().unwrap().as_ref().unwrap().memory_allocator.clone());
        ...
        let mut sets = Self::descriptor_sets(vulkan_data_reference.read().unwrap().as_ref().unwrap().desc_allocator.clone(), set_layouts, voxel_buffer.clone(), camera_data_buffer.clone(), render_buffer_data.view.clone());


        // Main render loop
        let mut recreate_swapchain = false;
        let mut prev_frame_end = Some(sync::now(vulkan_data_reference.read().unwrap().as_ref().unwrap().device.clone()).boxed());
        ...
        event_loop.run(move |event, _, control_flow| {
            match event {
                ...
                Event::WindowEvent { event: WindowEvent::Focused(true), ..} => {
                    window_focused = true;
                    vulkan_data_reference.read().unwrap().as_ref().unwrap().window.set_cursor_visible(false);
                }

                Event::WindowEvent { event: WindowEvent::Focused(false), ..} => {
                    window_focused = false;
                    vulkan_data_reference.read().unwrap().as_ref().unwrap().window.set_cursor_visible(true);
                }

                Event::WindowEvent { event: WindowEvent::CursorMoved { position, ..}, .. } => {
                    if window_focused {
                        let dim = vulkan_data_reference.read().unwrap().as_ref().unwrap().window.inner_size();
                        ...
                        vulkan_data_reference.read().unwrap().as_ref().unwrap().window.set_cursor_position(center).unwrap();
                    }
                }
                ...
                Event::RedrawEventsCleared => {
                    ...
                    if recreate_swapchain {
                        ...
                        sets = Self::descriptor_sets(vulkan_data_reference.read().unwrap().as_ref().unwrap().desc_allocator.clone(), new_set_layouts, voxel_buffer.clone(), camera_data_buffer.clone(), render_buffer_data.view.clone());
                    ...
                    let (img_index, suboptimal, acquire_future) = match acquire_next_image(vulkan_data_reference.read().unwrap().as_ref().unwrap().swapchain.clone(), None) {
                        Ok(r) => r,
                    ...
                    let mut builder = AutoCommandBufferBuilder::primary(vulkan_data_reference.read().unwrap().as_ref().unwrap().cmd_allocator.clone().as_ref(), vulkan_data_reference.read().unwrap().as_ref().unwrap().queue.queue_family_index(), CommandBufferUsage::OneTimeSubmit,).unwrap();
                    builder.bind_pipeline_compute(compute_pipline.clone())
                       ...
                       .copy_buffer_to_image(CopyBufferToImageInfo::buffer_image(render_buffer_data.buffer.clone(), render_buffer_data.image.clone())).unwrap()
                       .dispatch([vulkan_data_reference.read().unwrap().as_ref().unwrap().images[0].dimensions().width() / 8, vulkan_data_reference.read().unwrap().as_ref().unwrap().images[0].dimensions().height() / 8, 1]).unwrap()
                       .copy_image_to_buffer(CopyImageToBufferInfo::image_buffer(render_buffer_data.image.clone(), render_buffer_data.buffer.clone())).unwrap()
                       .copy_image(CopyImageInfo::images(render_buffer_data.image.clone(), vulkan_data_reference.read().unwrap().as_ref().unwrap().images[img_index as usize].clone())).unwrap();

                    let command_buffer = builder.build().unwrap();
                    let future = sync::now(vulkan_data_reference.read().unwrap().as_ref().unwrap().device.clone()).join(acquire_future)
                                                                    .then_execute(vulkan_data_reference.read().unwrap().as_ref().unwrap().queue.clone(), command_buffer).unwrap().then_swapchain_present(vulkan_data_reference.read().unwrap().as_ref().unwrap().queue.clone(), SwapchainPresentInfo::swapchain_image_index(vulkan_data_reference.read().unwrap().as_ref().unwrap().swapchain.clone(), img_index)).then_signal_fence_and_flush();
                    ...
                    match future {
                        ...
                        Err(FlushError::OutOfDate) => {
                            recreate_swapchain = true;
                            prev_frame_end = Some(sync::now(vulkan_data_reference.read().unwrap().as_ref().unwrap().device.clone()).boxed());
                        }
...
impl WinitVulkanHandler {
    pub fn init() -> Self {
        WinitVulkanHandler { vulkan_data: Arc::new(RwLock::new(None)) }
    }
    ...
    fn descriptor_sets(desc_allocator: Arc<StandardDescriptorSetAllocator>, set_layouts: &[Arc<DescriptorSetLayout>], voxel_buffer: Arc<CpuAccessibleBuffer<[VoxelData]>>, misc_buffer: Arc<CpuAccessibleBuffer<Camera>>, img_view: Arc<dyn ImageViewAbstract>) -> Vec<Arc<PersistentDescriptorSet>> {
        let mut sets = vec![];
