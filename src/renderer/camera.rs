...
impl Camera {
    fn create_camera_to_world_space(
        forward: Vector3<f32>,
        up: Vector3<f32>,
        position: Vector3<f32>,
    ) -> Matrix4<f32> {
        let mut camera_to_world = Matrix4::look_at_rh(
    ...
    pub fn update_camera_dir(&mut self, target: Vector3<f32>, up_ref: Vector3<f32>) {
        ...
        self.camera_to_world_mat =
            Self::create_camera_to_world_space(new_forward, new_up, self.position);
    }

    pub fn camera_move(
        &mut self,
        input: Vector3<f32>,
        forward_vec: Vector3<f32>,
        delta_time: f32,
        movement_speed: f32,
    ) {
