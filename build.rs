...
fn main() {
    ...
    for i in 0..17 {
        recursed += &recursion.replace("NN", &i.to_string()).replace("MMM", &(i+1).to_string()).replace("\n", &format!("\n{}", "    ".repeat(i+1)));
    }
    recursed += "\n";
    for i in 0..17 {
        recursed += &format!("{}}}\n", "    ".repeat(17-i));
    ...
    write("src/renderer/window_handler/winit_vulkan_handler/shader.rs", shader).unwrap();
}