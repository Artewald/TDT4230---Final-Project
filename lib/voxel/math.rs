...
pub fn mul_vector3(vec1: Vector3<f32>, vec2: Vector3<f32>) -> Vector3<f32> {
    Vector3::new(vec1.x * vec2.x, vec1.y * vec2.y, vec1.z * vec2.z)
}
...
pub fn vec2_one_d_overlapping(vec1: Vector2<u32>, vec2: Vector2<u32>) -> bool {
    (vec2.x < vec1.x && vec2.y > vec1.y)
        ...
        || vec2_one_d_in_range(
            Vector2::new(vec1.x as f32, vec1.y as f32),
            Vector2::new(vec2.x as f32, vec2.y as f32),
        )
        || vec2_one_d_in_range(
            Vector2::new(vec2.x as f32, vec2.y as f32),
            Vector2::new(vec1.x as f32, vec1.y as f32),
...
pub fn distance_between_points(point_1: Vector3<f64>, point_2: Vector3<f64>) -> f32 {
    ((point_1.x - point_2.x).powi(2)
        + (point_1.y - point_2.y).powi(2)
        + (point_1.z - point_2.z).powi(2))
    .sqrt() as f32
}
